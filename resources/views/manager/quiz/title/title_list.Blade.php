    ...
    <div class="row justify-content-center">
        <div class="col-3">
            <button type="button" class="btn btn-primary w-100 p-3 mt-5" 
                onclick="location.href='{{ url('/manager/quiz/title/title_create') }}' ">
                問題集作成</a>
        </div>      
    ...
    @foreach($titles as $title)
        ...
        <tbody>  
            <tr>
                ...
                <a href="{{ route('manager.quiz.title.check', ['id' =>$title->id]) }}">    
                    <button type="button" class="btn btn-success">詳細</button>                
                </td>
                <td class="align-middle">  
                <a href="{{ route('manager.quiz.title.q_set', ['id' =>$title->id]) }}">    
                    <button type="button" class="btn btn-info">設定</button></a>
                </td>
                <td class="align-middle">  
                <a href="{{ route('manager.quiz.title.title_edit', ['id' =>$title->id]) }}">
                    <button type="button" class="btn btn-warning">編集</button></a>
                </td>
                
                <td class="align-middle"> 
                    <form action="{{ route('manager.quiz.title.title_delete', ['id' =>$title->id]) }}" method="POST">
                    @csrf
