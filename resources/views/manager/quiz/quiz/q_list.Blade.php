    ...
    <div class="row justify-content-center">
        <div class="col-3">
            <button type="button" class="btn btn-primary w-100 p-3 mt-5" 
                onclick="location.href='{{ url('/manager/quiz/quiz/q_create') }}' ">
                新規問題作成</a>
        </div>      
            <div class="row justify-content-center">
        ...
        <tbody>  
            <tr>
                ...
                <td class="align-middle">
                    <a href="{{ route('manager.quiz.quiz.q_check', ['id' =>$quiz->id]) }}">    
                    <button type="button" class="btn btn-success">詳細</button>                
                </td>

                <td class="align-middle">  
                <a href="{{ route('manager.quiz.quiz.q_category_edit', ['id' =>$quiz->id]) }}">
                    <button type="button" class="btn btn-info ms-3">変更</button></a></td>
                </td>


                <td class="align-middle">  
                <a href="{{ route('manager.quiz.quiz.q_edit', ['id' =>$quiz->id]) }}">
                    <button type="button" class="btn btn-warning">編集</button></a></td>
                </td>
                
                <td class="align-middle"> 
                    <form action="{{ route('manager.quiz.quiz.q_delete', ['id'=>$quiz->id]) }}" method="POST">
                    @csrf
